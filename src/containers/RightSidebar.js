...
function RightSidebar(){
    ...
    return(
        <div className={" fixed overflow-hidden z-20 bg-gray-900 bg-opacity-50 inset-0 transform ease-in-out " + (isOpen ? " transition-opacity opacity-100 duration-500 translate-x-0  " : " transition-all delay-500 opacity-0 translate-x-full  ")}>
            ...
            <section className={ "w-80 md:w-96  right-0 absolute bg-base-100 h-full shadow-xl delay-400 duration-500 ease-in-out transition-all transform  " + (isOpen ? " translate-x-0 " : " translate-x-full ")}>
                    ...
                    <div className="relative  pb-5 flex flex-col  h-full">
                        ...
                        <div className="navbar flex pl-4 pr-4 shadow-md ">
                            <button className="float-left btn btn-circle btn-outline btn-xs" onClick={() => close()}>
                            <XMarkIcon className="h-3 w-3 "/>
                            </button>
                            <span className="mr-4 font-bold text-xl">{header}</span>
                        </div>
